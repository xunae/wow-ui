...
function mod:Screech(args)
	self:Message2("swarm", "orange", L.swarm_message, L.swarm_icon)
	self:PlaySound("swarm", "alarm")
end

function mod:GreatHeal(args)
	self:Message2(23954, "red", CL.casting:format(args.spellName))
...
function mod:MindFlay(args)
	self:Message2(23954, "yellow")
	self:PlaySound(23954, "info")
end

function mod:CHAT_MSG_MONSTER_YELL(_, msg)
	if msg:find(L.bomb_trigger, nil, true) then
		self:Message2("bomb", "orange", L.bomb_message, L.bomb_icon)
