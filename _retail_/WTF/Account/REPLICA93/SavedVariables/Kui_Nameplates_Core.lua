
KuiNameplatesCoreSaved = {
	["2162_PERSONAL_FRAME_SIZE_TRANSITION"] = true,
	["226_AURAS_TRANSITION"] = true,
	["226_CLASSPOWERS_Y"] = true,
	["216_HEALTH_TRANSITION"] = true,
	["226_TARGET_SIZE"] = true,
	["profiles"] = {
		["default"] = {
			["cvar_clamp_top"] = 0.0299999937415123,
			["cvar_overlap_v"] = 0.699999988079071,
			["auras_icon_normal_size"] = 26,
			["bot_vertical_offset"] = 0,
			["auras_hide_all_other"] = true,
			["castbar_height"] = 10,
			["auras_cd_x"] = 0,
			["castbar_unin_colour"] = {
				1, -- [1]
				0, -- [2]
				0, -- [3]
			},
			["cvar_occluded_mult"] = 0.8,
			["name_vertical_offset"] = 2,
			["frame_target_size"] = false,
			["auras_icon_squareness"] = 1,
			["auras_enabled"] = false,
			["fade_non_target_alpha"] = 0.6,
			["bar_animation"] = 2,
			["execute_enabled"] = false,
			["bar_texture"] = "Blizzard Raid Bar",
			["fade_speed"] = 0.1000000014901161,
			["raid_icon_y"] = 1,
			["auras_show_purge"] = false,
			["raid_icon_x"] = 27,
			["target_arrows"] = true,
			["target_glow_colour"] = {
				0.3019607843137255, -- [1]
				0.7019607843137254, -- [2]
				1, -- [3]
				0.5, -- [4]
			},
			["frame_width_minus"] = 98,
			["frame_width"] = 125,
			["ignore_uiscale"] = true,
			["frame_height_target"] = 8,
			["font_style"] = 3,
			["nameonly"] = false,
			["auras_cd_point"] = 5,
			["fade_all"] = true,
			["frame_height"] = 10,
			["cvar_enable"] = true,
			["auras_cd_y"] = 0,
			["font_size_normal"] = 12,
			["frame_width_target"] = 125,
			["auras_offset"] = 16,
			["cvar_personal_show_combat"] = false,
			["global_scale"] = 1.5,
			["frame_glow_size_target"] = 4,
			["font_face"] = "Arial Narrow",
			["target_arrows_size"] = 20,
			["fade_conditional_alpha"] = 0.300000011920929,
			["castbar_name_vertical_offset"] = -2,
			["castbar_colour"] = {
				1, -- [1]
				1, -- [2]
				0, -- [3]
			},
			["nameonly_no_font_style"] = true,
		},
	},
}
