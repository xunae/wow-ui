...
function mod:RottingRegurgitation(args)
	self:Message("breath", "yellow", 18609, 262292) -- Breath (Rotting Regurgitation)
...
function mod:ShockwaveStomp(args)
	self:Message(args.spellId, "orange", CL.count:format(args.spellName, stompCount))
...
function mod:TrashChuteVisualState()
	...
	if (self:Mythic() and trashCount % 3 == 2) or (not self:Mythic() and trashCount % 2 == 1) then -- Small add
		self:Message(262364, "cyan", CL.incoming:format(self:SpellName(-17867)))
	...
	elseif (self:Mythic() and trashCount % 3 == 1) then -- Big Add
		self:Message(262364, "cyan", CL.incoming:format(self:SpellName(-18565)))
		self:PlaySound(262364, "long")
