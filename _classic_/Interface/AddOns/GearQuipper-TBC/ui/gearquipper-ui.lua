    ...
    if paperDollFrame then
        ...
        GameTooltip:HookScript("OnTooltipSetItem", function(self)
            if GQ_OPTIONS[c.OPT_SHOWITEMTOOLTIP] and self and self:GetOwner() then
                ...
                if ownerName and not c:TableContains(c:GetSlotInfo(), ownerName:gsub("Character", "")) then
    ...
    if not slotId then
        ...
        if not slot.gqTex then
            ...
            slot:HookScript("OnEnter", function(self)
                c:ShowExpectedItemTooltip(self);
            end);
...
function c:ShowExpectedItemTooltip(slot)
    -- experimental
    if not gqToolTip then
        gqToolTip = CreateFrame("GameTooltip", "GQ_SlotTooltip", UIParent, "GameTooltipTemplate");
    end

    gqToolTip:SetOwner(slot, "ANCHOR_RIGHT");
    if slot.gqInfo then
        gqToolTip:AddLine(slot.gqInfo);
        gqToolTip:Show();
        GameTooltip:SetOwner(gqToolTip, "ANCHOR_BOTTOM");
