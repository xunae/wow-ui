...
local function SendRareInfo(name, time)
    SendChatMessage(format("Next Rare: %s in %s at %s", name, time, C_Map.GetUserWaypointHyperlink()), "CHANNEL", nil, 1)
end
...
function ICECROWN_RARES_TRACKER_MODULE:Update()
    ...
    if nextRareRemainTimeSec > rareStepWoUnattack then
        ...
        nextRareRemainTime = KT.SecondsToTime(nextRareRemainTimeSec - rareStepWoUnattack).." (unattackable now)"
    ...
    self:AddObjective(block, 0, "Next Rare:", nil, nil, OBJECTIVE_DASH_STYLE_HIDE_AND_COLLAPSE, OBJECTIVE_TRACKER_COLOR["Label"])
...
function M:OnEnable()
    ...
    ICECROWN_RARES_TRACKER_MODULE:SetHeader(header, "Icecrown Rares")
