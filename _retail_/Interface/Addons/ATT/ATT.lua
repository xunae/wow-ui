...
local Badge = GetSpellInfo(345228)
local Emblem = GetSpellInfo(345231)
...
local dbPVPtrinkets = {
 ...
 {	["ability"] = Badge, ["id"] = 345228, ["cooldown"] = 120, ["sname"] = "badge",},
 {	["ability"] = Emblem, ["id"] = 345231, ["cooldown"] = 90, ["sname"] = "emblem",},
 }
...
local dbVisibility = {
 ...
 {	["ability"] = "inraid" , ["sname"] = "Show in Raid",},
...
function ATT:InspectIsReady(guid)
    ...
    if isInspect then
        ...
        dbInspect[inspectedUnit][45] = guid
    else
...
function ATT:EnqueueInspect()
    ...
    for i=1, 4 do
       local unit = "party"..i
       local guid = UnitGUID(unit)
       if activeGUIDS[guid] == nil and not (dbInspect[unit] and dbInspect[unit][45] == guid) then self:UpdateAnchors(unit) end
...
for k=1,12 do
    local itemLink = GetInventoryItemLink(unit, k) or 0
    itemName, _, itemQuality = GetItemInfo(itemLink)
    for a, b in pairs(dbLegeModif) do
        if itemQuality == 5 and itemName == GetSpellInfo(b.ability) then
            dbInspect[41] =  {["lege"] = b.mod , ["legecd"] = b.cooldown }
            break
        end
    end
end
  ...
  --  C_SpecializationInfo.GetSpellsDisplay(63)
    --Trinkets
    for abilityIndex, abilityTable in pairs(dbPVPtrinkets) do
       ...
       if ability == nil and id == 214027 and (item1 == (184054 or 181816) or item2 == (184054 or 181816)) and db.Trinkets[sname] then ability = AdaptedName end --adapt fix
       if ability == nil and id == 196029 and (item1 == (184053 or 181335) or item2 == (184053 or 181335)) and db.Trinkets[sname] then ability = RelentlessName end --relent fix
    ...
    for abilityIndex, abilityTable in pairs(dbExtraTrinkets) do
        ...
        icon.texture:SetTexture(select(10,GetItemInfo(sname)))
        icon.GUID = anchor.GUID
        icon.ability = ability
        icon.abilityID = id
        icon.cooldown = cooldown
    ...
    if db.abilities[class]["ALL"] then
        for abilityIndex, abilityTable in pairs(db.abilities[class]["ALL"]) do
            ...
            if modif and GetSpellSubtext(modif.mod) == GetSpellSubtext(204336) and not pvpactive then modif.mod = nil end
            ...
            if spellStatus == "DISABLED" or self:FindAbilityByName(dbInspect, ability) or (tcheck and tcheck.isPVP and not pvpactive) or (SpellText == GetSpellSubtext(204336) and not tcheck) then ability = nil; end
            ...
            if dbInspect[41] and dbInspect[41].lege and id == dbInspect[41].lege then cooldown = cooldown - dbInspect[41].legecd end
...
function ATT:ApplyAnchorSettings()
    ...
    if (db.arena and instanceType == "arena") or
        ...
        (db.inraid and instanceType == "raid") or
        (db.inbg and instanceType == "pvp" ) or
        (db.outside and instanceType == "none")
    then
    if db.biggroup and (not isPartyGr or GetNumGroupMembers() > 5) then ATTIcons:Hide() ; self:Hide()
...
function ATT:UNIT_AURA(unit)
    ...
    if not fd and isFWW["fd"..unit] == unit then C_Timer.After(0.12, function() self:StartCooldown(GetSpellInfo(5384), unit, 5384); isFWW["fd"..unit] = nil; end ) end
    self:Dampening()
end
    ...
    if racecheck and racecheck.cdshare then
        ...
        for k,icon in ipairs(anchor.icons) do
            ...
            elseif icon.ability == RelentlessName then
                if (spellName == EveryMan) then cooldown = 180
...
function ATT:COMBAT_LOG_EVENT_UNFILTERED()
    ...
    if self:GetUnitByGUID(sourceGUID) and ((event == "SPELL_CAST_SUCCESS") or ((event == "SPELL_AURA_APPLIED") and (spellName == Stoneform or spellName == Fireblood ))) then
...
function ATT:UNIT_SPELLCAST_SUCCEEDED(unit,_,spellid)
    ...
    if not unit then return end
    -- Feign Death workaround fix
    if spellid == 5384 then self:StartCooldown(GetSpellInfo(5384), unit, 5384); isFWW["fd"..unit] = unit end
end
...
function ATT:PLAYER_EQUIPMENT_CHANGED(item)
    ...
    if item ~= (13 and 14) then return end
    self:InspectPlayer()
end
...
function ATT:UNIT_POWER_FREQUENT(unit,power)
    ...
    if ATT:ValidUnit(unit) and power == "COMBO_POINTS" then
        ...
        C_Timer.After(0.03, function()
            if dbInspect and dbInspect[42] and dbInspect[42].cpower ~= powerunit then dbInspect[42].cpower = powerunit end
        end )
    end
end
...
local function ATT_OnLoad(self)
    ...
    ATTDB = { abilities = ATTdefaultAbilities, scale = 1, iconOffsetX = 5 , iconOffsetY = 2 , offsetX = 0, offsetY = 0, attach = 0, showSelf = true, outside = true, arena = true, glow = true, Trinkets = {}  } end
    ATTDB = ATTDB or { abilities = ATTdefaultAbilities, scale = 1, iconOffsetX = 5 , iconOffsetY = 2 , offsetX = 0, offsetY = 0, attach = 0, showSelf = true, outside = true, arena = true, glow = true, Trinkets = {} }
    db = ATTDB
...
function ATT:CreateOptions()
    ...
    local biggroup = panel:MakeToggle(
        'name', 'Max 5 Man',
        'description', 'Hide icons in groups with\nmore than 5 members',
        'default', false,
    ...
    authordesc:SetText("|cffffff00Version:|r |cff33ff99v"..ATTversion.."|r by |cffffff00izy|r  ( Contact: @curseforge.com or twitch.tv/imedia )")
    ...
    local showPVETrinket = panel:MakeButton(
        ...
        'func', function()
            ...
            for abilityIndex, abilityTable in pairs(dbExtraTrinkets) do
                ...
                if abilityIndex == 1 then  btn[#btn+1] = 1 button:SetPoint("LEFT") end
                --local abilitytexture = self:FindAbilityIcon(ability, id)
                local abilitytexture = select(10,GetItemInfo(sname))
                button.Text:SetText(" |T"..abilitytexture..":17|t " ..GetItemInfo(sname))
                button:SetHitRectInsets(0, -button.Text:GetWidth() , 0, 0)
                button:SetPoint("TOPLEFT",btn[#btn],"BOTTOMLEFT")
                button:SetChecked(db["Trinkets"][sname])
                button:SetScript("OnClick",  function() db["Trinkets"][sname] = button:GetChecked(); self:UpdateGroup();  end)
    ...
    local selectVisibility = panel:MakeButton(
        ...
        'func', function()
            ...
            for abilityIndex, abilityTable in pairs(dbVisibility) do
                ...
                button:SetWidth(30);
                if abilityIndex == 1 then btn[#btn+1] = 1 button:SetPoint("LEFT")  end
                button.text:SetText(" |T"..GetItemIcon(166879)..":17|t "..sname)
