	...
	----------------------------------------------------------------------
	-- 	Leatrix Maps 8.3.38 (7th October 2020)
	----------------------------------------------------------------------
	...
	do
		...
		if gametocversion and gametocversion > 90000 then
			LeaMapsLC.SL = true
			LeaMapsLC.BackdropTemplate = "BackdropTemplate"
		end
	end
	...
	function LeaMapsLC:MainFunc()
		...
		if LeaMapsLC["NoMapBorder"] == "On" then
			...
			if LeaMapsLC.SL then
				...
				qborder = QuestMapFrame:CreateTexture(nil, "BACKGROUND")
				qborder:SetTexture("Interface\\ChatFrame\\ChatFrameBackground")
				qborder:SetPoint("TOPLEFT", 1, 8)
				qborder:SetPoint("BOTTOMRIGHT", 6, -6)
				qborder:SetVertexColor(0, 0, 0, 0.7)
			...
			hooksecurefunc("QuestLogQuests_Update", function()
				if LeaMapsLC.SL then
					...
					QuestScrollFrame.Background:SetHeight(465)
				end
			end)
			...
			hooksecurefunc(WorldMapFrame, "SynchronizeDisplayState", function()
				if WorldMapFrame.isMaximized then
					...
					if LeaMapsLC.SL then
						...
						maxBtn:SetPoint("CENTER", WorldMapFrame, "TOPRIGHT", -114, -84)
					end
					closeBox:ClearAllPoints()
					if LeaMapsLC.SL then
						closeBox:SetPoint("CENTER", WorldMapFrame, "TOPRIGHT", -110, -84)
					else
						closeBox:SetPoint("CENTER", WorldMapFrame, "TOPRIGHT", -70, -84)
					end
				else
					...
					if LeaMapsLC.SL then
						...
						maxBtn:SetPoint("CENTER", WorldMapFrame, "TOPLEFT", 600, -84)
					end
					closeBox:ClearAllPoints()
					if LeaMapsLC.SL then
						closeBox:SetPoint("CENTER", WorldMapFrame, "TOPLEFT", 600, -84)
					else
						closeBox:SetPoint("CENTER", WorldMapFrame, "TOPLEFT", 640, -84)
					end
				end
			end)
			...
			local function thirdPartyFunc(thirdPartyAddOn)
				if thirdPartyAddOn == "ElvUI" and not LeaMapsLC.SL then
					-- ElvUI
					if WorldMapFrame.backdrop then WorldMapFrame.backdrop:Hide() end
					QuestMapFrame:ClearAllPoints()
					QuestMapFrame:SetPoint("TOPRIGHT", WorldMapFrame, "TOPRIGHT", -5, -66)
					QuestMapFrame:SetHeight(458)
					QuestMapFrame.DetailsFrame:ClearAllPoints()
					QuestMapFrame.DetailsFrame:SetPoint("BOTTOMLEFT", QuestMapFrame, "BOTTOMLEFT", 2, -7)
					qborder:ClearAllPoints()
					qborder:SetPoint("TOPLEFT", 1, 4)
					qborder:SetPoint("BOTTOMRIGHT", 6, -13)
				end
			end
		...
		if LeaMapsLC["ShowIcons"] == "On" then
			...
			if LeaMapsLC.SL then
				...
				--[[Bastion]] 			PinData[1533] = PinData[1533] or {}; 	tinsert(PinData[1533], {40.0, 55.3, L["The Necrotic Wake"], L["Dungeon"], 1182, dnTex, nil, nil, nil, nil})
				--[[Bastion]] 			PinData[1533] = PinData[1533] or {}; 	tinsert(PinData[1533], {58.4, 28.7, L["Spires of Ascension"], L["Dungeon"], 1186, dnTex, nil, nil, nil, nil})
				--[[Maldraxxus]] 		PinData[1536] = PinData[1536] or {}; 	tinsert(PinData[1536], {59.2, 64.8, L["Plaguefall"], L["Dungeon"], 1183, dnTex, nil, nil, nil, nil})
				--[[Maldraxxus]] 		PinData[1536] = PinData[1536] or {}; 	tinsert(PinData[1536], {53.0, 53.0, L["Theater of Pain"], L["Dungeon"], 1187, dnTex, nil, nil, nil, nil})
				--[[Ardenweald]] 		PinData[1565] = PinData[1565] or {}; 	tinsert(PinData[1565], {68.7, 66.1, L["De Other Side"], L["Dungeon"], 1188, dnTex, nil, nil, nil, nil})
				--[[Ardenweald]] 		PinData[1565] = PinData[1565] or {}; 	tinsert(PinData[1565], {35.4, 53.9, L["Mists of Tirna Scithe"], L["Dungeon"], 1184, dnTex, nil, nil, nil, nil})
				--[[Revendreth]] 		PinData[1525] = PinData[1525] or {}; 	tinsert(PinData[1525], {78.3, 49.1, L["Halls of Atonement"], L["Dungeon"], 1185, dnTex, nil, nil, nil, nil})
				--[[Revendreth]] 		PinData[1525] = PinData[1525] or {}; 	tinsert(PinData[1525], {50.9, 29.9, L["Sanguine Depths"], L["Dungeon"], 1189, dnTex, nil, nil, nil, nil})
				--[[Revendreth]] 		PinData[1525] = PinData[1525] or {}; 	tinsert(PinData[1525], {46.2, 41.2, L["Castle Nathria"], L["Raid"], 1190, rdTex, nil, nil, nil, nil})
				...
				do
					...
					if PinData[pMapID] then
						...
						for i = 1, count do
							...
							if not pinInfo[7] or pinInfo[7] and not pinInfo[8] and IsQuestFlaggedCompleted(pinInfo[7]) or pinInfo[7] and pinInfo[8] and IsQuestFlaggedCompleted(pinInfo[7]) and not IsQuestFlaggedCompleted(pinInfo[8]) then
			...
			function LeaMapsGlobalPinMixin:OnClick()
	...
	if LeaMapsLC.SL then
		...
		LeaMapsLC:MakeCB(PageF, "ShowIcons", "Show dungeons and raids", 225, -112, true, "If checked, dungeons and raids will be shown.")
