...
-- Constants
KT_OBJECTIVE_TRACKER_DOUBLE_LINE_HEIGHT = 0
KT_QUEST_DASH = "- "

-- Blizzard Constants
MAX_WATCHABLE_QUESTS = MAX_QUESTLOG_QUESTS
...
local function ResetScale()
	...
	if parentScale > 0.64 and parentScale < 0.65 then
		local scale = ConvertPixelsToUI(1, parentScale)
		KTF:SetScale(scale)
	end
end
...
local function SetHooks()
	...
	function QUEST_TRACKER_MODULE:OnBlockHeaderClick(block, mouseButton)  -- R
		if ( IsModifiedClick("CHATLINK") and ChatEdit_GetActiveWindow() ) then
			...
			ChatEdit_InsertLink(gsub(questName, " *(.*)", "%1"));
...
StaticPopupDialogs[addonName.."_WowheadURL"] = {
	...
	OnShow = function(self)
		...
		self.editBox:SetText("http://"..lang.."tbc.wowhead.com/"..self.text.text_arg1.."="..self.text.text_arg2)
...
function KT:OnEnable()
	...
	self.screenWidth, self.screenHeight = string.match(GetCVar("gxWindowedResolution"), "(%d+)x(%d+)")
